			...
			(new CDiv())
				...
				->addItem([
					...
					(new CFormGrid())
						...
						->addItem([
							new CLabel([_('Multiplier'),
								makeHelpIcon([
									_('Add a multiplier to the data set. Useful for converting from bits -> bytes, or values over time back to absolute values to generate estimates or totals per some time interval'),
									BR(), BR(),
									_('Examples:'),
									BR(),
									(new CList([
										new CListItem([
											_(' - bps -> total bytes: Override axes tab with static unit of "B". To get total bytes per day, set this box to 10800 (1 * 86400 / 8). Set aggregation function to avg or sumOfAvg and aggregation interval to 1d')
										]),
										new CListItem([
											_(' - packets -> pps: Overrides axes tab with static unit of "!pps". Set aggregation function to sum and aggregation interval to a minimum of 1h. Set multiplier to 0.0002777 (1 / 3600). 3600 -> 1h, 7200 -> 2h, etc...')
										]),
										new CListItem([
											_(' - pps -> total packets: Override axes tab with static unit of "!pkts". To get total packets per day, set this box to 86400 (1 * 86400)')
										])
									]))
								])
							], $field_name.'_'.$row_num.'_custom_multiplier'),
							new CFormField(
								(new CTextBox($field_name.'['.$row_num.'][custom_multiplier]', $value['custom_multiplier']))
									->setId($field_name.'_'.$row_num.'_custom_multiplier')
						...
						->addItem([
							new CLabel(
								[
									...
									(new CSpan(
										makeWarningIcon(_('Use sumOfAvg when summing rate-based metrics (i.e. bps, pps), not sum'))
									))->setId($field_name.'_'.$row_num.'-warn-if-sum-aggregation')
								],
							...
							new CFormField(
								(new CSelect($field_name.'['.$row_num.'][aggregate_function]'))
									...
									->addOptions(CSelect::createOptionsFromArray([
										...
										CWidgetFieldDataSet::AGGREGATE_SUMAVG => 'sumOfAvg',
										CWidgetFieldDataSet::AGGREGATE_ITEMCOUNT => 'itemCount'
									]))
									->setWidth(ZBX_TEXTAREA_SMALL_WIDTH)
							)
						])
						...
						->addItem([
							...
							new CFormField(
								(new CRadioButtonList($field_name.'['.$row_num.'][aggregate_grouping]',
									...
									->addValue(_('Item name'), CWidgetFieldDataSet::GRAPH_AGGREGATE_BY_NAME)
