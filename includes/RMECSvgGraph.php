...
use CSvg,
	...
	CSvgGraphGrid,
	CSvgGraphAxis,
	CSvgGraphWorkingTime,
	CSvgGraphPercentile,
	CSvgGraphSimpleTrigger,
	CMathHelper,
	CSvgGraphProblems,
	CSvgGraphClipArea,
	CSvgRect,
...
use Modules\RMESvgGraph\Includes\{
	RMECSvgGraphMetricsLine,
	RMECSvgGraphMetricsPoint,
	RMECSvgGraphMetricsBar
};

class RMECSvgGraph extends CSvg {
	...
	private $show_suppress_zero_values;
	...
	// RME Add-ons to support graph line display
	// toggle when clicking entries in the legend
	private $left_y_min_js_override;
	private $left_y_max_js_override;
	private $left_y_units_js_override;
	private $right_y_min_js_override;
	private $right_y_max_js_override;
	private $right_y_units_js_override;
	private $lines_hidden_js_override;
	...
	public function __construct(array $options) {
		...
		$this->show_suppress_zero_values = $options['displaying']['show_suppress_zero_values'];
		...
		$this->left_y_min_js_override = $options['axes']['left_y_min_js_override'];
		$this->left_y_max_js_override = $options['axes']['left_y_max_js_override'];
		$this->left_y_units_js_override = $options['displaying']['left_y_units_js_override'];
		$this->right_y_min_js_override = $options['axes']['right_y_min_js_override'];
		$this->right_y_max_js_override = $options['axes']['right_y_max_js_override'];
		$this->right_y_units_js_override = $options['displaying']['right_y_units_js_override'];
		$this->lines_hidden_js_override = $options['displaying']['lines_hidden_js_override'];
	...
	public function addMetrics(array $metrics): RMECSvgGraph {
	...
	public function addSimpleTriggers(array $simple_triggers): RMECSvgGraph {
		$this->simple_triggers = $simple_triggers;

		return $this;
	}

	public function addProblems(array $problems): RMECSvgGraph {
	...
	private function calculateDimensions(): void {
		...
		if ($this->left_y_units === null) {
			if ($this->left_y_units_js_override !== '') {
				$this->left_y_units = $this->left_y_units_js_override;
			}
			else {
				$this->left_y_units = '';
				foreach ($this->metrics as $metric) {
		...
		if ($this->right_y_units === null) {
			if ($this->right_y_units_js_override !== '') {
				$this->right_y_units = $this->right_y_units_js_override;
			}
			else {
				$this->right_y_units = '';
				foreach ($this->metrics as $metric) {
		...
		if ($this->left_y_min_js_override !== '') {
			$this->min_value_left = $this->left_y_min_js_override;
		}

		if ($this->left_y_max_js_override !== '') {
			$this->max_value_left = $this->left_y_max_js_override;
		}

		if ($this->right_y_min_js_override !== '') {
			$this->min_value_right = $this->right_y_min_js_override;
		}

		if ($this->right_y_max_js_override !== '') {
			$this->max_value_right = $this->right_y_max_js_override;
		}
					...
					foreach ($point as $type => $value) {
						...
						if ($value < $min_value || $value > $max_value) {
							$y = $metric['options']['type'] == SVG_GRAPH_TYPE_POINTS
								? RMECSvgGraphMetricsPoint::Y_OUT_OF_RANGE
	...
	private function drawMetricsLine(): void {
		foreach ($this->metrics as $index => $metric) {
			...
			$this->addItem(new RMECSvgGraphMetricsLine($metric_paths, $metric));
		}
	}
	...
	private function drawMetricsPoint(): void {
		foreach ($this->metrics as $index => $metric) {
			if ($metric['options']['type'] == SVG_GRAPH_TYPE_POINTS && array_key_exists($index, $this->paths)) {
				...
				$this->addItem(new RMECSvgGraphMetricsPoint(array_column(reset($this->paths[$index]), $approximation), $metric));
			}
		}
	}
	...
	private function drawMetricsBar(): void {
		foreach ($this->bar_paths as $metric_index => $path) {
			$this->addItem(new RMECSvgGraphMetricsBar($path, $this->metrics[$metric_index]));
		}
	}
