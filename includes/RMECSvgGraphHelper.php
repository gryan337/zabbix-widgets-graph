...
use Modules\RMESvgGraph\Actions\{
	WidgetView
};
...
class RMECSvgGraphHelper {
		...
		foreach ($metrics as $data => $metric) {
			if (!in_array($metric['options']['custom_multiplier'], $omitted)) {
				...
				foreach ($metric['points'] as $tick => $point) {
					foreach ($point as $thing => $value) {
						$new_m_points['points'][$tick][$thing] = $value * $metric['options']['custom_multiplier'];
					}
				}
				if ($new_m_points) {
					$metrics[$data]['points'] = $new_m_points['points'];
				}
			}
		}
		...
		if ($display_options['show_suppress_zero_values'] == WidgetView::SVG_GRAPH_SUPPRESS_ZEROS_TRUE) {
			foreach ($metrics as $data => $metric) {
				...
				foreach ($metric['points'] as $point) {
					if (array_key_exists($approx, $point)) {
						if ($point[$approx] != 0 && $point[$approx] != 0.0) {
							$all_zero = false;
							break;
						}
					}
	...
	private static function limitMetricsDisplayed(array &$metrics, array $options): void {
		foreach ($options['data_sets'] as $index => &$data_set) {
			$reduce = false;

			if ($data_set['stacked'] == SVG_GRAPH_STACKED_ON &&
					($data_set['aggregate_function'] == AGGREGATE_NONE ||
					($data_set['aggregate_function'] != AGGREGATE_NONE &&
					($data_set['aggregate_grouping'] == GRAPH_AGGREGATE_BY_ITEM || $data_set['aggregate_grouping'] == CWidgetFieldDataSet::GRAPH_AGGREGATE_BY_NAME)))
				) {
				$max_metrics = SVG_GRAPH_MAX_NUMBER_OF_METRICS <= 50
					? SVG_GRAPH_MAX_NUMBER_OF_METRICS
					: 50;
				$reduce = true;
			}
			elseif ($data_set['aggregate_function'] == AGGREGATE_NONE ||
					($data_set['aggregate_function'] != AGGREGATE_NONE &&
					($data_set['aggregate_grouping'] == GRAPH_AGGREGATE_BY_ITEM || $data_set['aggregate_grouping'] == CWidgetFieldDataSet::GRAPH_AGGREGATE_BY_NAME))
				) {
				$max_metrics = SVG_GRAPH_MAX_NUMBER_OF_METRICS <= 100
					? SVG_GRAPH_MAX_NUMBER_OF_METRICS
					: 100;
				$reduce = true;
			}
			...
			if ($reduce) {
				foreach ($metrics as $mindex => $metric) {
					if ($metric['data_set'] == $index) {
						if ($max_metrics <= 0) {
							unset ($metrics[$mindex]);
						}
						$max_metrics--;
					}
				}
	...
	private static function getMetricsData(array &$metrics, int $width): void {
		...
		foreach ($metrics as $index => &$metric) {
			...
			if ($metric['options']['data_set_label'] !== '') {
				...
				$metrics_to_update[$index] = $metric['itemid'];

				// Process batch when it reached the limit
				if (count($items_to_resolve) >= $batch_size) {
					...
					foreach ($metrics_to_update as $idx => $itemid) {
						$resolved_value = $resolved[$itemid]['label'];

						if ($resolved_value !== null && $resolved_value !== '' && $resolved_value !== '*UNKNOWN*') {
							$metrics[$idx]['name'] = $resolved_value;
						}
						else if ($resolved_value === $metrics[$idx]['options']['data_set_label']) {
							$metrics[$idx]['name'] = $metrics[$idx]['hosts'][0]['name'].NAME_DELIMITER.$metrics[$idx]['options']['data_set_label'];
						}
						else {
							$metrics[$idx]['name'] = $metrics[$idx]['hosts'][0]['name'].NAME_DELIMITER.$metrics[$idx]['name'];
						}
					}
		...
		if (!empty($items_to_resolve)) {
			...
			foreach ($metrics_to_update as $index => $itemid) {
				$resolved_value = $resolved[$itemid]['label'];

				if ($resolved_value !== null && $resolved_value !== '' && $resolved_value !== '*UNKNOWN*') {
					$metrics[$index]['name'] = $resolved_value;
				}
				else if ($resolved_value === $metrics[$index]['options']['data_set_label']) {
					$metrics[$index]['name'] = $metrics[$index]['hosts'][0]['name'].NAME_DELIMITER.$metrics[$index]['options']['data_set_label'];
				}
				else {
					$metrics[$index]['name'] = $metrics[$index]['hosts'][0]['name'].NAME_DELIMITER.$metrics[$index]['name'];
				}
			}
		}
	...
	private static function getMetricsAggregatedData(array &$metrics, int $width, array $data_sets,
		...
		foreach ($metrics as $metric_num => &$metric) {
			...
			if ($metric['options']['aggregate_grouping'] == GRAPH_AGGREGATE_BY_ITEM ||
					$metric['options']['aggregate_grouping'] == CWidgetFieldDataSet::GRAPH_AGGREGATE_BY_NAME) {
				if ($legend_aggregation_show) {
					$ds_func = RMECItemHelper::getAggregateFunctionName($metric['options']['aggregate_function']);
					if ($data_sets[$dataset_num]['data_set_label'] !== '') {
						if ($resolved_value !== null && $resolved_value !== '' && $resolved_value !== '*UNKNOWN*') {
							$name = $ds_func.'('.$resolved_value.')';
						}
						else if ($resolved_value === $metric['options']['data_set_label']) {
							$name = $ds_func.'('.$data_sets[$dataset_num]['data_set_label'].')';
						}
						else {
						...
						$name = RMECItemHelper::getAggregateFunctionName($metric['options']['aggregate_function']).
			...
			if (!array_key_exists($dataset_num, $dataset_metrics)) {
				...
				if ($metric['options']['aggregate_grouping'] == GRAPH_AGGREGATE_BY_DATASET) {
					...
					if ($data_sets[$dataset_num]['dataset_type'] != CWidgetFieldDataSet::DATASET_TYPE_PATTERN_ITEM) {
						$metric['options']['color'] = '#'.$data_sets[$dataset_num]['color'][0];
					}
				}
		...
		foreach ($metrics as &$metric) {
			...
			$historyManager = new \Modules\RMESvgGraph\Includes\RMECHistoryManager();
			$result = $historyManager->getAggregationByInterval(
			...
			elseif (in_array($period, range(2592001,5184000))) {
				$approximation_tick_delta = ($period / $metric['options']['aggregate_interval']) > ($width / 4)
					? ceil($period / ($width / 4))
					: 0;
			}
			else {
				$approximation_tick_delta = ($period / $metric['options']['aggregate_interval']) > ($width / 8)
					? ceil($period / ($width / 8))
					: 0;
			}

			/**
			Round to the nearest integer divisible by one hour.
			One hour is the trends interval so this will prevent the
			approximation tick delta from bleeding across trends buckets
			and cause possible anomalous data points.
			**/
			$remainder = fmod($approximation_tick_delta, 3600);
			if ($remainder != $approximation_tick_delta) {
				$approximation_tick_delta = ($remainder) >= (3600 / 2)
					? $approximation_tick_delta += (3600 - $remainder)
					: $approximation_tick_delta -= $remainder;
			}
			...
			foreach ($result as $points) {
				...
				$sumavg_points = [];
				$is_sum_avg = (bool) ($metric['options']['aggregate_function'] == CWidgetFieldDataSet::AGGREGATE_SUMAVG);

				foreach ($points['data'] as $point) {
					if ($is_sum_avg && array_key_exists('value', $point)) {
						$sumavg_points[] = $point['value'];
					}

					if ($point['tick'] > ($tick + $approximation_tick_delta)) {
						$tick = $point['tick'];
						if ($is_sum_avg) {
							$point['value'] = CMathHelper::safeAvg($sumavg_points);
							$sumavg_points = [];
						}
					}
					else {
			...
			$final_points = [];
			$tick = 0;
			foreach ($metric_points as $time => $values) {
				if ($time > ($tick + $approximation_tick_delta)) {
					$tick = $time;
				}

				foreach ($values as $value) {
					$final_points[$tick][] = $value;
				}
			}

			$metric_points = $final_points;
			...
			switch ($metric['options']['aggregate_function']) {
				...
				case CWidgetFieldDataSet::AGGREGATE_ITEMCOUNT:
					foreach ($metric_points as $tick => $points) {
						$itemids = array_column($points, 'itemid');
						$metric['points'][$tick] = array_fill_keys($approximation_functions,
				...
				case AGGREGATE_SUM:
				case CWidgetFieldDataSet::AGGREGATE_SUMAVG:
					foreach ($metric_points as $tick => $points) {
						$metric['points'][$tick] = array_fill_keys($approximation_functions,
	...
	private static function getLegend(array $metrics, array $legend_options): ?RMECSvgGraphLegend {
		...
		return (new RMECSvgGraphLegend($items))
