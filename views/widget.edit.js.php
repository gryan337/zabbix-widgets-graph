...
use Modules\RMESvgGraph\Includes\CWidgetFieldDataSet;
...
window.widget_form = new class extends CWidgetForm {
	...
	_addSumWarning(dataset) {
		const aggregation_function = dataset.querySelector(`[name="ds[${dataset.dataset.set}][aggregate_function]"]`);
		const warn_icon = document.getElementById('ds_' + dataset.dataset.set + '-warn-if-sum-aggregation');
		if (warn_icon !== null) {
			warn_icon.style.display = aggregation_function.value == <?= AGGREGATE_SUM ?> ? '' : 'none';
		}
	}
	...
	_cloneDataset() {
		...
		this._addSumWarning(cloned_dataset);
	}
