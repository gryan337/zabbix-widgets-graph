...
function getDisplayOptionsTab(CWidgetFormView $form, array $fields): CDiv {
	...
	$hintbox_sort = $form->registerField(
		(new CWidgetFieldRadioButtonListView($fields['hintbox_sort']))
			->setFieldHint(
				makeHelpIcon([
					_('Specify how to sort the values in the tooltip hintbox on graph hover'), BR(),
					_(' - Name: sorts ascendingly, by name'), BR(),
					_(' - Value: sorts the numeric values ascendingly or descendingly')
				])
			)
	);
	...
	$suppress_zero_values = $form->registerField(
		(new CWidgetFieldCheckBoxView($fields['suppress_zero_values']))
			->setFieldHint(
				makeHelpIcon([
					_('Hide metrics where all the values are 0 for the selected time range')
				])
			)
	);
	...
	return (new CDiv())
		...
		->addItem(
			(new CFormGrid())
				...
				->addItem([
					$hintbox_sort->getLabel(),
					new CFormField($hintbox_sort->getView())
				])
				->addItem([
		...
		->addItem(
			(new CFormGrid())
				...
				->addItem([
					$suppress_zero_values->getLabel(),
					new CFormField($suppress_zero_values->getView())
				])
		);
}
